...
def sun_position_from_date_and_time(geo_lon, geo_lat, year, month, day, hour, minute=0, second=0, timezone='UTC',
                                    debug=False):
...
def airmass(sun_alt, return_value_below_horizon=False):
    ...
    if return_value_below_horizon:
        airmass[sel] = 1000 * (
                    0.15 + abs(sun_alt[sel]))  # Very bad, but still getting worse for even lower Sun, for solvers
    else:
    ...
    airmass[sel] = (1.002432 * np.sin(sun_alt[sel]) ** 2 + 0.148386 * np.sin(sun_alt[sel]) + 0.0096467) / \
                       (np.sin(sun_alt[sel]) ** 2 * np.sin(sun_alt[sel]) + 0.149864 * np.sin(
                           sun_alt[sel]) ** 2 + 0.0102963 * np.sin(sun_alt[sel]) + 0.000303978)
...
def extinction_factor(airmass, return_value_below_horizon=False):
    ...
    sel = (airmass > 38.2)
    if return_value_below_horizon:
        ext_fac[sel] = np.sqrt(sys.float_info.max) * (
                       0.15 + airmass[sel])  # Very bad, but still getting worse for even higher airmass, for solvers
    else:
...
def diffuse_radiation_projection_perez87(doy, sun_alt, surf_incl, theta, beam_norm, dif_horiz):
    ...
    if arrSize == 1:  # Scalar
        ...
        Mair[sun_alt < 10 * d2r] = 1 / (
                    np.sin(sun_alt[sun_alt < 10 * d2r]) + 0.15 * (sun_alt[sun_alt < 10 * d2r] * r2d + 3.885) ** (
                -1.253))
    ...
    if arrSize == 1:  # Scalar
        ...
        epsilon[(dif_horiz <= 0) & (
                    beam_norm > 0)] = 99  # Some direct daylight: x/0 = large -> completely clear, should be >11 for last row of Table 1
        epsilon[dif_horiz > 0] = (dif_horiz[dif_horiz > 0] + beam_norm[dif_horiz > 0]) / dif_horiz[
            dif_horiz > 0]  # Overcast: epsilon ~ 1,  clear: epsilon -> infinity
    ...
    if arrSize == 1:  # Scalar
        ...
        chiC[theta < pio2 + alpha] = psiH[theta < pio2 + alpha] * psiC[theta < pio2 + alpha] * np.sin(
            psiC[theta < pio2 + alpha] * alpha)
...
def clearsky_bird(sun_alt, i_ext=1353, sun_dist=1, press=1013, uo=0.34, uw=1.42, ta5=0.2661, ta3=0.3538, ba=0.84,
                  k1=0.1, rg=0.2):
    ...
    To = 1 - 0.1611 * Xo * (1 + 139.48 * Xo) ** (-0.3035) - 0.002715 * Xo / (
                1 + 0.044 * Xo + 0.0003 * Xo ** 2)  # Transmittance of ozone absorptance
    ...
    Tw = 1 - 2.4959 * Xw / (
                (1 + 79.034 * Xw) ** 0.6828 + 6.385 * Xw)  # Transmittance of water-vapor absorptance - Tw = 1-Aw

    # Daily turbidity:
    Tau = 0.2758 * ta3 + 0.35 * ta5  # Broadband turbidity: aerosol optical depth from surface in a vertical column
    Ta = np.exp(
        -Tau ** 0.873 * (1 + Tau - Tau ** 0.7088) * AM ** 0.9108)  # Transmittance of aerosol absorptance and scattering
    Taa = 1 - k1 * (1 - AM + AM ** 1.06) * (
                1 - Ta)  # Transmittance of aerosol absorptance - this does not agree with tabulated values from the paper (except for AM~1).  When I substitute their values for Taa, everything matches perfectly.  Error in their formula, or in their table?
